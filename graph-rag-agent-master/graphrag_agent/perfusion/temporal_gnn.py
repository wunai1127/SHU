...
class TemporalGNNConfig:
    ...
    配置类 - 支持两种参数命名方式：
    1. 详细命名: temporal_input_size, temporal_hidden_size, etc.
    2. 简化命名: num_features, hidden_dim, num_layers, dropout (用于 train_gnn.py)
    """
    ...
    # 简化参数名 (用于兼容 train_gnn.py 的调用方式)
    # Simplified parameter names (for compatibility with train_gnn.py)
    hidden_dim: int = None       # Maps to temporal_hidden_size
    num_layers: int = None       # Maps to temporal_num_layers
    dropout: float = None        # Maps to temporal_dropout
    num_features: int = None     # Maps to temporal_input_size

    # 图编码器开关
    # 当没有真实知识图谱数据时，可以禁用图编码器，只使用时序编码器
    ...
    def __post_init__(self):
        ...
        if self.hidden_dim is not None:
            self.temporal_hidden_size = self.hidden_dim
            self.graph_hidden_size = self.hidden_dim
            self.fusion_hidden_size = self.hidden_dim
        if self.num_layers is not None:
            self.temporal_num_layers = self.num_layers
            self.graph_num_layers = self.num_layers
        if self.dropout is not None:
            self.temporal_dropout = self.dropout
        if self.num_features is not None:
            self.temporal_input_size = self.num_features
...
class TemporalPerfusionGNN(nn.Module):
    ...
    def __init__(self, config: Optional[TemporalGNNConfig] = None):
        ...
        self.use_graph_encoder = self.config.use_graph_encoder
        ...
        if self.use_graph_encoder:
            ...
            # 不使用图编码器时，只使用时序特征
            # When not using graph encoder, only use temporal features
            self.graph_encoder = None
            fusion_input_size = self.config.temporal_hidden_size
    ...
    def forward(
        ...
        # 2. Encode graph structure (if enabled)
        # 编码图结构，捕捉医学知识关联（如果启用）
        if self.use_graph_encoder and graph_data is not None:
            ...
            # 不使用图编码器，直接使用时序表示
            # Not using graph encoder, use temporal representation directly
            h_fused = self.fusion(h_temporal)

        # 4. Generate predictions
        # 生成预测结果
        # 注意：quality_score 输出 0-1，训练时与标签 (quality_score/100) 一致
        # 如需 0-100 分数，调用方自行乘以 100
        quality_score = self.quality_head(h_fused)  # Output 0-1, NOT scaled to 0-100
        ...
        return {
            'quality_score': quality_score.squeeze(-1),  # 0-1 range
...
class FeatureNormalizer:
    ...
    def transform(self, x: np.ndarray) -> np.ndarray:
        """
        Transform/normalize numpy array (sklearn-style API)
        归一化 numpy 数组（兼容 sklearn 风格的 API）

        Args:
            x: numpy array of shape [time_steps, features] or [batch, time_steps, features]
    ...
    def inverse_transform(self, x: np.ndarray) -> np.ndarray:
        """
        Inverse transform (denormalize) numpy array
        反归一化 numpy 数组
